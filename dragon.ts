...
function newWorld() {
    return {
        ...
        get : function(col : number ,row : number) : number {
        ...
        set : function(col : number , row : number , value : number) {
            if (row >= 0 && row < global_rows && col >=0 && col < global_cols) {
    ...
    for (let row = 0 ; row < global_rows ; row++) {
        for (let col = 0 ; col < global_cols ; col++) {
            if (Math.random() < density) {
                //world.set(col,row,1);
                //world.set(col,row,Math.floor(Math.random()* 4 + 1));
    ...
    for (let row = 0 ; row < global_rows ; row++) {
        for (let col = 0 ; col < global_cols ; col++) {
            if (world.get(col,row) > 0 ) {
                if (Math.random() > 0.5) player.set(col,row,-1); else player.set(col,row,1);
            }
        }
    }
}
...
function clusterContainsHeadAndBody() {
    ...
    for (let row = 0; row < global_rows ; row++) {
        for (let col = 0 ; col  < global_cols; col++) {
            if (cluster.get(col,row) == 1 && world.get(col,row) == 1  ) contains_body = true;
            if (cluster.get(col,row) == 1 && world.get(col,row) == 3  ) contains_head = true;         
        }
    }
    if (contains_head && contains_body) return true; else return false;
}
...
function clusterNeighbor(check_col : number , check_row : number ) {
...
// function fixedIndex(x : number, modulus : number) {
...
function isArmor(piece_code : number) {
    if (piece_code == 3) return true; else return false;
}
function isBody(piece_code : number ) {
    if (piece_code == 1) return true; else return false;
}
